  ...
  return (
    <div className='flex flex-col text-white py-2'>
      ...
      <div className='flex px-3 space-x-4 overflow-x-auto w-screen' >
        {data?.map((items)=>(
          <SmallCard key={items?.data?.details[1].text} title={items?.data?.title[0]?.text} artist={items?.data?.details[0].text} 
            image={items?.data?.image?.sub?.url} alt={items?.data?.image?.sub?.alt} isPodcast uid={items?.uid}/>
        ))}
      </div>
    </div>
  )
